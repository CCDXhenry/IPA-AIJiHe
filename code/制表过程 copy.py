...
def 渠道档案判断处理(df_待调剂明细数据, df_渠道档案报表):
    ...
    # 添加渠道档案相关列
    df_渠道档案_匹配 = df_渠道档案报表[['渠道编码', '发展月结算范围', '协议起始日', '协议终止日', '渠道状态']].drop_duplicates()
    ...
    df_待调剂明细数据.loc[终止日期条件, '结算原因'] += '|解约:协议终止日待稽核|'
    ...
    if 发展月结算范围_存在记录.any():
        # 转换日期格式
        df_待调剂明细数据['发展月结算范围'] = pd.to_datetime(
            df_待调剂明细数据['发展月结算范围'].astype(str),
            format='%Y%m',
            errors='coerce'
        )
