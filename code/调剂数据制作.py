...
######主程序######
#数据表
basic_path = rf"C:\project\python_project\AI+市场工具&稽核\AI+市场工具&稽核第一期需求\选项目-制表-得出结论\AI稽核-调剂"
# 获取当前年月
...
#     if 0 <= index < len(all_file_info):
#         file_info = all_file_info[index]
#         薪酬类型 = file_info['薪酬类型']
#         工单级别 = file_info['工单级别']
#         统计月份 = file_info['统计月份']
#         政策分期小项编码 = file_info['政策分期小项编码']
#         渠道归属县市 = file_info['渠道归属县市']
#         文件路径 = file_info['文件路径']
#         return 薪酬类型, 工单级别, 统计月份, 政策分期小项编码, 渠道归属县市, 文件路径
#     else:
#         return None

# # 使用示例
# for i in range(len(all_file_info)):
#     result = get_file_info(i)
#     if result:
#         薪酬类型, 工单级别, 统计月份, 政策分期小项编码, 渠道归属县市, 文件路径 = result
#         print(f"处理第{i+1}个文件: {文件路径}")

for info_index in range(len(all_file_info)):
    file_info = all_file_info[info_index]
    薪酬类型 = file_info['薪酬类型']
    工单级别 = file_info['工单级别']
    统计月份 = file_info['统计月份']
    政策分期小项编码 = file_info['政策分期小项编码']
    ...
    if 薪酬类型 == '营销支撑服务费管理':
        ...
        薪酬类型_index = 1 if 薪酬类型 == '代办渠道费用' else 11
    ...
    if 工单级别 == '县级报账(县级工单)':
        ...
        渠道归属县市_index = 县市索引映射.get(渠道归属县市)
    else:
        工单级别_index = 2
    logging.info(f"处理第{info_index+1}个文件: {文件路径}")